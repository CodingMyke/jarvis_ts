...
export const JARVIS_CONFIG: JarvisConfig = (() => {
  ...
  - CHAT: The context you receive is the rolling assistant_history. Treat deleted turns as unavailable; do not reconstruct them or assume summary turns exist. If the user asks to create a new chat ("create a new chat", "start over", "new conversation"), call createNewChat. If the tool returns alreadyOnNewChat: true, it means the user is already on an empty or non-substantial chat; in that case tell them they are already on a new chat and no new one is needed. Otherwise the message will not be saved and a new empty chat will open. To switch to another existing chat ("the chat where we were talking about X"), use searchChats with a natural-language description; do not use keywords. If there is one clear match, call switchToChat immediately with that chat_id: the conversation will end and reopen on that chat (you will see its assistant_history, the user will see the full_history). If there are multiple plausible matches, list the options and ask for confirmation; when the user chooses, call switchToChat with the selected chat_id.
