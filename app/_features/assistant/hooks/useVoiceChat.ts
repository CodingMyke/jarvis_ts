...
export function useVoiceChat(options?: UseVoiceChatOptions): UseVoiceChatReturn {
  ...
  const [listeningMode, setListeningMode] = useState<AssistantSessionState>("idle");
  ...
  const transitionSession = useCallback((event: AssistantSessionEvent) => {
    setListeningMode((state) => transitionAssistantSession(state, event));
  }, []);

  const updateChatMetadata = useCallback((chat?: {
    id?: string;
    title?: string | null;
    created_at?: string;
    ...
    if (chat.created_at !== undefined) {
      chatCreatedAtRef.current = chat.created_at ?? null;
    }
    if (chat.last_activity_at !== undefined) {
      chatLastActivityAtRef.current = chat.last_activity_at ?? null;
    }
  }, []);

  const resetChatMetadata = useCallback(() => {
  ...
  const saveConversation = useCallback(async (currentMessages: Message[]) => {
    ...
    if (cid) {
      ...
      try {
        const data = await appendChatTurns(cid, newTurns);
        if (data?.chat) {
          lastSavedTurnCountRef.current = turns.length;
          updateChatMetadata(data.chat);
        }
      } catch (err) {
    ...
    try {
      const data = await createChat(turns);
  ...
  const goToIdle = useCallback(() => {
    ...
    stopWakeWordLifecycle(wakeWordRef.current);
    ...
    transitionSession("STOP_LISTENING");
  }, [saveConversation, transitionSession]);
  ...
  const goToWakeWord = useCallback(() => {
    ...
    setConnectionState("disconnected");
    currentMessageIdRef.current = { user: null, ai: null };
    transitionSession("END_CONVERSATION");
  ...
  const clearConversationAndReconnect = useCallback(() => {
    ...
    setConnectionState("disconnected");
    currentMessageIdRef.current = { user: null, ai: null };
    transitionSession("DELETE_CHAT");
    connectToGeminiRef.current(`Ciao ${JARVIS_CONFIG.assistantName}`);
  }, [clearConversation, transitionSession]);
  ...
  const performDeleteChat = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      const result = await deleteChatById(cid);
      if (!result.ok) {
        return { success: false, error: result.message ?? "Eliminazione fallita" };
      }
      setTimeout(() => clearConversationAndReconnect(), DELETE_AND_RECONNECT_DELAY_MS);
      return { success: true };
    } catch (e) {
  ...
  const createNewChatAndReconnect = useCallback(async () => {
    ...
    try {
      ...
      if (!data?.success || !data.chat?.id) {
        console.error("[useVoiceChat] POST /api/chats (createNewChat) failed");
        return;
      }
      updateChatMetadata(data.chat);
      setMessages([]);
  ...
  }, [saveConversation, transitionSession, updateChatMetadata]);
  ...
  const connectToGemini = useCallback(async (initialMessage?: string) => {
    ...
    try {
      ...
      const cid = chatIdRef.current;
      if (cid) {
        const data = await fetchChatById(cid);
        if (data?.success && data.chat) {
          updateChatMetadata({
            id: cid,
            title: data.chat.title,
            created_at: data.chat.created_at,
            last_activity_at: data.chat.last_activity_at,
          });
      ...
      const client = new VoiceChatClient({
        ...
        onStateChange: (state: ConnectionState) => {
          ...
          if (state === 'disconnected' && clientRef.current) {
            ...
            setOutputAudioLevel(0);
            currentMessageIdRef.current = { user: null, ai: null };
            transitionSession("CONNECTION_LOST");
            wakeWordRef.current?.resume();
          }
        },
        ...
        onSwitchToChat: async (newChatId: string) => {
          const data = await fetchChatById(newChatId);
          if (!data?.success) {
            return { success: false, error: data?.message ?? "Chat non trovata" };
          }
          setTimeout(
      ...
      transitionSession("CONNECT_SUCCESS");
      ...
      if (hasInitialMessage) {
        ...
        if (!chatIdRef.current) {
          ...
          try {
            const data = await createChat([firstTurn]);
            if (data?.success && data.chat?.id) {
              updateChatMetadata(data.chat);
  ...
  }, [handleTranscript, saveConversation, options, goToIdle, goToWakeWord, scheduleInactivityTimeout, performDeleteChat, performDeleteChatById, switchToChatAndReconnect, createNewChatAndReconnect, resetChatMetadata, transitionSession, updateChatMetadata]);
  ...
  const startListening = useCallback(() => {
    ...
    wakeWordRef.current = startWakeWordLifecycle({
      ...
      onError: (voiceError) => {
        console.error("[useVoiceChat] wake word error:", voiceError);
        setError(voiceError);
      },
    });

    transitionSession("START_LISTENING");
  }, [listeningMode, connectToGemini, transitionSession]);
