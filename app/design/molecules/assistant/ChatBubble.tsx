...
function ChatBubbleComponent({
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className="max-w-[86%] wrap-break-word px-4 py-2.5 text-foreground transition-[background-color,border-color] duration-(--transition-medium) ease-(--easing-smooth)"
        style={{
...
export const ChatBubble = memo(ChatBubbleComponent, (previous, next) => {
  return (
    previous.message.id === next.message.id
    && previous.message.text === next.message.text
    && previous.message.thinking === next.message.thinking
    && previous.message.isUser === next.message.isUser
