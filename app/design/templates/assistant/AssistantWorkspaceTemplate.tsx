...
export function AssistantWorkspaceTemplate({
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-background p-6">
      {!!errorMessage && (
        <div className="absolute left-0 right-0 top-0 z-20 rounded-app border-b border-line-danger bg-danger-surface-strong px-4 py-3 text-center text-sm text-danger-copy shadow-overlay backdrop-blur-sm">
          {errorMessage}
        </div>
      )}
