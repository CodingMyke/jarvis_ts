...
import { searchSemanticMemoriesByContent } from "@/app/_features/memory";
...
/**
 * POST: ricerca semantica nelle memorie semantiche.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await searchSemanticMemoriesByContent(auth.supabase, auth.userId, query, matchCount);
    if (!result.success) {
      return NextResponse.json(
    ...
    console.error("[POST /api/memory/semantic/search]", error);
