...
export const JARVIS_CONFIG: JarvisConfig = (() => {
  ...
  - CHAT: Il contesto che ricevi è la history compattata (assistant_history). Tratta i turni di riassunto come verità consolidata; non ricostruire messaggi eliminati. Per passare a un'altra chat ("la chat in cui parlavamo di X") usa il tool searchChats con una descrizione in linguaggio naturale; non usare keyword. Se c'è un solo match chiaro proponi di passare a quella chat; se multipli elenca le opzioni e chiedi conferma.
